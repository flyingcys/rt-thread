...
int32_t platform_in_isr(void)
{
    ...
    return ((rt_interrupt_get_nest() != 0UL) ? 1: 0);
}

#if defined(CPU_M0)
static void IPC_M0_IRQHandler(int irq, void *arg)
{
...
#elif defined(CPU_D0)
static void IPC_D0_IRQHandler(int irq, void *arg)
{
    uint32_t irqStatus = BL_RD_REG(IPC2_BASE, IPC_CPU0_IPC_IRSRR);


    BL_WR_REG(IPC2_BASE, IPC_CPU0_IPC_ICR, irqStatus);    
}

#elif defined(CPU_LP)
static void IPC_LP_IRQHandler(int irq, void *arg)
{
    uint32_t irqStatus = BL_RD_REG(IPC1_BASE, IPC_CPU0_IPC_IRSRR);


    BL_WR_REG(IPC1_BASE, IPC_CPU0_IPC_ICR, irqStatus);    
}
#endif
...
// #endif
    ...
    BL_WR_REG(IPC2_BASE, IPC_CPU1_IPC_IUSR, 0xffffffff);
    bflb_irq_enable(IPC_D0_IRQn);
#elif defined(CPU_LP)
    bflb_irq_attach(IPC_LP_IRQn, IPC_LP_IRQHandler, NULL);
    BL_WR_REG(IPC1_BASE, IPC_CPU0_IPC_IUSR, 0xffffffff);
...
// #endif
#if defined(CPU_M0)
    bflb_irq_disable(IPC_M0_IRQn);
    bflb_irq_attach(IPC_M0_IRQn, NULL, NULL);
#elif defined(CPU_D0)
    bflb_irq_disable(IPC_D0_IRQn);
    bflb_irq_attach(IPC_D0_IRQn, NULL, NULL);
#elif defined(CPU_LP)
    bflb_irq_disable(IPC_LP_IRQn);
    bflb_irq_attach(IPC_LP_IRQn, NULL, NULL);
#else
#error "unsupport chip" 
#endif
    }
