...
static volatile struct rpmsg_lite_instance *rpmsg_client = NULL;
...
void rpmsg_lite_remote_entry(void *param)
{
    ...
    rpmsg_client = rpmsg_lite_remote_init((void *)XRAM_RINGBUF_ADDR, RL_PLATFORM_BL808_LINK_ID, RL_NO_FLAGS);
    RT_ASSERT(rpmsg_client != RL_NULL);

    rpmsg_lite_wait_for_link_up(rpmsg_client, RT_WAITING_FOREVER);
    rt_kprintf("link is up!\r\n");

    ipc_queue = rpmsg_queue_create(rpmsg_client);
    
    ipc_ept = rpmsg_lite_create_ept(rpmsg_client, LOCAL_EPT_ADDR, ept_rx_cb, ipc_queue);
